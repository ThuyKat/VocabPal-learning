...
const wordConverter = {
  ...
  fromFirestore(snapshot: DocumentSnapshot, options?: SnapshotOptions): Word {
    const data = snapshot.data(options || {}) as Omit<Word, 'id' | 'createdAt' | 'updatedAt'> & {
      createdAt: any;
      updatedAt: any;
    }; // type assertion to bypass TypeScript compile time error
    return {
      id: snapshot.id,
      word: data.word,
...
export async function createWord(
  ...
  //check if word has existed first
  const words = await getWordsByUser(word.userId);
  const duplicate = words.find(
    (wordEl) => wordEl.word.toLocaleLowerCase() === word.word.toLocaleLowerCase()
  );
  if (duplicate) {
    console.log('word existed');
    return null;
  }
  //add to the db
